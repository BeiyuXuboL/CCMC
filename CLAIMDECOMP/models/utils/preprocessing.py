...
def concat_claim_gpt_rationale2(input_text, examples):
    res = []
    if "confuse_evidence" not in examples.keys():
        print("val/test dataset")
        return concat_claim_gpt_rationale(input_text, examples)
    for claim, rationale in zip(
            input_text,
            examples['confuse_evidence']
    ):
        res.append(
            f'{claim} {SEP_TK} {rationale}'
        )
...
def concat_claim_gpt_rationale3(input_text, examples):
    res = []
    if "noise_evidence" not in examples.keys():
        print("val/test dataset")
        return concat_claim_gpt_rationale(input_text, examples)
    for claim, rationale in zip(
            input_text,
            examples['noise_evidence']
    ):
        res.append(
            f'{claim} {SEP_TK} {rationale}'
        )

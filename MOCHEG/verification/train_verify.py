...
from verification.util.data_util import get_data_loader, get_loss_weights, get_data_loader_inference
from transformers import CLIPProcessor, CLIPModel, BertTokenizer, BertModel
from verification.util.constants import *
from verification.model import  MultiModal2, MultiView
...
def train_loop( tuner_config,config_kwargs):
    ...
    if args.mode in ["train","hyper_search"]:
        ...
        dataloaders,train_label_statistic=get_data_loader_inference(args.train_txt_dir,args.train_img_dir,args.val_txt_dir,args.val_img_dir,args.test_txt_dir, args.test_img_dir,processor, None)

    model = MultiView(bert = None, clip = clip_model, concat_hidden_size = 256, final_hidden_size = 64, 
...
def train_phase(accum_iter,model,dataloader,device,optimizer ,criterion,temperature):
    print("Train" + ":")
    live_loss = 0
    model.train()
    with tqdm(dataloader, unit="batch") as tepoch:
        for batch_idx, (batch,confusing_batch,random_batch) in enumerate(tepoch):
            labels = batch['label'].to(device)
            claim_img_encod = batch['claim_img_encod']
            output_G, output_C, output_R, _ = model(claim_img_encod, confusing_batch['claim_img_encod'], random_batch['claim_img_encod'],device)
            loss2 = criterion(output_G, labels)
            loss3 = criterion(output_C, labels)
            loss4 = criterion(output_R, labels)
...
def val_phase(accum_iter,model,dataloader,device, best_valid_f1,args,is_save_predict,epoch,best_epoch  ):
    ...
    model.eval()
    with tqdm(dataloader, unit="batch") as tepoch:
        for batch_idx, (batch,_,_) in enumerate(tepoch):
            labels = batch['label'].to(device)
            claim_img_encod = batch['claim_img_encod']
            output = model.inference( claim_img_encod,device)
